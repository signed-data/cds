...
class CDSVocab:
    ...
    # energy.brazil — ANP fuel prices (combustivel)
    FUEL_PRICE_RETAIL             = content_type_uri("energy.brazil", "fuel.price-retail")
    # health.brazil — epidemiological surveillance
    HEALTH_FLU_SURVEILLANCE       = content_type_uri("health.brazil", "flu.surveillance")
    HEALTH_SRAG_NOTIFICATION      = content_type_uri("health.brazil", "srag.notification")
    # education.brazil — IBGE / PNAD
    EDUCATION_LITERACY_RATE       = content_type_uri("education.brazil", "literacy.rate")
    EDUCATION_ENROLLMENT          = content_type_uri("education.brazil", "school.enrollment")
    EDUCATION_INDICATORS          = content_type_uri("education.brazil", "education.indicators")
    # climate.brazil — INMET station data
    CLIMATE_STATION_HISTORY       = content_type_uri("climate.brazil", "station.history")
    CLIMATE_STATION_CURRENT       = content_type_uri("climate.brazil", "station.current")
    CLIMATE_STATION_LIST          = content_type_uri("climate.brazil", "station.list")
...
class CDSSources:
    ...
    ANP                  = source_uri("dados.anp.gov.br.v1")
    FIOCRUZ_INFOGRIPE    = source_uri("info.gripe.fiocruz.br.v1")
    OPENDATASUS          = source_uri("opendatasus.saude.gov.br.v1")
    IBGE_SIDRA           = source_uri("servicodados.ibge.gov.br.sidra.v3")
    INMET                = source_uri("apitempo.inmet.gov.br.v1")
