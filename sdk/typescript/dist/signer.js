...
export function generateKeypair(privateKeyPath = "keys/private.pem", publicKeyPath = "keys/public.pem") {
...
const PSS_OPTS = { padding: 6, saltLength: -2 };
export class CDSSigner {
    privateKey;
    issuer;
    constructor(privateKey, issuer = "signed-data.org") {
...
export class CDSVerifier {
    ...
    verify(event) {
        if (!event.integrity)
            throw new Error("Event has no integrity metadata.");
        const canonical = event.canonicalBytes();
        ...
        if (!valid)
            throw new Error("Signature invalid — event may be tampered.");
        return true;
    }
}
