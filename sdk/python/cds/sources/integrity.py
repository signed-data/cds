"""
SignedData CDS — Integrity Brazil Fetcher
...
Phase 1 exposes the consolidated CEIS/CNEP/CEPIM sanction check.
"""
...
from cds.sources.integrity_models import (
    IntegrityContentTypes,
    SanctionRecord,
...
# The Portal da Transparência API uses the `chave-api-dados` header.
...
def _normalise_ceis(raw: dict[str, Any]) -> SanctionRecord:
    """Parse a CEIS record from Portal da Transparência into SanctionRecord.

    CEIS fields (observed): dataInicioSancao, dataFimSancao, tipoSancao.descricao,
    orgaoSancionador.nome, fundamentacao, nomeSancionado, cnpjSancionado,
    razaoSocial, informacoesAdicionais.
    """
...
def _normalise_cnep(raw: dict[str, Any]) -> SanctionRecord:
    """Parse a CNEP record (Lei Anticorrupção) into SanctionRecord."""
    return SanctionRecord(
        registry="CNEP",
        cnpj=str(raw.get("cnpjSancionado") or "").strip(),
        nome_sancionado=str(raw.get("nomeSancionado") or raw.get("razaoSocial") or "").strip(),
...
def _normalise_cepim(raw: dict[str, Any]) -> SanctionRecord:
    """Parse a CEPIM record (nonprofits impedidas) into SanctionRecord."""
    organ = (
        _nested_get(raw, "orgaoSuperior", "nome")
        or _nested_get(raw, "concedente", "nome")
    )
    return SanctionRecord(
        registry="CEPIM",
        cnpj=str(raw.get("cnpjSancionado") or "").strip(),
        nome_sancionado=str(raw.get("nomeSancionado") or raw.get("razaoSocial") or "").strip(),
        sanction_type=_nested_get(raw, "motivo", "descricao") or raw.get("motivo"),
        start_date=raw.get("dataInicio") or raw.get("dataReferencia"),
        end_date=raw.get("dataFim"),
        sanctioning_organ=organ,
        legal_basis=raw.get("fundamentacao"),
        raw=raw,
    )
...
def _parse_consolidated(
    ...
    cepim_raw: list[dict[str, Any]],
) -> SanctionsConsolidated:
    ceis = [_normalise_ceis(r) for r in ceis_raw]
    cnep = [_normalise_cnep(r) for r in cnep_raw]
    cepim = [_normalise_cepim(r) for r in cepim_raw]
    total = len(ceis) + len(cnep) + len(cepim)
    return SanctionsConsolidated(
        cnpj=cnpj_bare,
        cnpj_formatted=_format_cnpj(cnpj_bare),
        sanction_found=total > 0,
        sanction_count=total,
        registries={"ceis": ceis, "cnep": cnep, "cepim": cepim},
        query_timestamp=query_ts,
    )


def _build_sanctions_summary(consolidated: SanctionsConsolidated) -> str:
    """Human-readable summary for a consolidated sanction check."""
    cnpj = consolidated.cnpj_formatted
    if not consolidated.sanction_found:
        return f"{cnpj}: sem sanções federais em CEIS, CNEP ou CEPIM."

    breakdown = []
    for key in ("ceis", "cnep", "cepim"):
...
class SanctionsFetcher:
    """
    Fetches and signs CEIS/CNEP/CEPIM sanction data on demand.
    ...
    async def fetch_consolidated(self, cnpj: str) -> CDSEvent:
        """Fetch consolidated sanction status (CEIS + CNEP + CEPIM) for a CNPJ.

        Validates the CNPJ check digits before the API call. Raises
        `ValueError` for invalid CNPJ and `httpx.HTTPStatusError` on upstream
        error. A partial upstream failure (e.g. CEIS 200 but CNEP 503) fails
        the whole call in v1.
        """
        ...
        async with httpx.AsyncClient(timeout=20, headers=headers) as client:
            ceis_resp, cnep_resp, cepim_resp = await asyncio.gather(
                client.get(
                ...
                client.get(
                    f"{PORTAL_TRANSPARENCIA_BASE}/cepim",
                    params={"cnpjSancionado": bare, "pagina": 1},
                ),
            )
        for resp in (ceis_resp, cnep_resp, cepim_resp):
        ...
        fingerprint = _fingerprint(
            ceis_resp.content + cnep_resp.content + cepim_resp.content
        )
